...
def analyze_image(req: https_fn.Request) -> https_fn.Response:
    try:
        ...
        print(result)
        os.remove(tmp_path)
        my_response = { "status": "success", "score": result["score"], "processedUrl": result["processed_image_url"], "letterGrade": result["letter_grade"], "message": "Image processed successfully" }
        ...
        print(e.code)
        if(e.code == "NO_JUMP"):
            my_response = { "status": "success", "score": 0, "processedUrl": image_url, "letterGrade": "F", "message": "No significant jump detected. Did you even get close?" }
            return https_fn.Response(json.dumps(my_response), status=200, headers={"Content-Type": "application/json"})
        else:
            my_response = { "status": "error", "score": 0, "processedUrl": image_url, "letterGrade": "F", "message": e.message }
            return https_fn.Response(json.dumps(my_response), status=400, headers={"Content-Type": "application/json"})
